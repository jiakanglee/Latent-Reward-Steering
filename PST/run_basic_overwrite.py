...
def main():
    ...
    if lm_head is None:
        lm_head = getattr(raw_model.model, 'lm_head', None)
    if lm_head is None:
    ...
    layer_module = raw_model.model.layers[args.sae_layer]
    ...
    for i in target_indices:
        try:
            ...
            with torch.no_grad():
                out_base = raw_model.generate(**inputs, max_new_tokens=args.max_token, do_sample=False)
            ...
            try:
                out_steer = raw_model.generate(**inputs, max_new_tokens=args.max_token, do_sample=False)

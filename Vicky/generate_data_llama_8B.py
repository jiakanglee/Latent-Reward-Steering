...
def load_dataset_by_name(dataset_name):
    ...
    if dataset_name == "math500":
        ...
        ds = load_dataset("nichenshun/gpqa_diamond")["train"]  # 198 题
        return [(ds[i], "gpqa_diamond") for i in range(len(ds))]
    else:
...
def main():
    ...
    parser.add_argument('--max_token', type=int, default=-1,
        help='Max new tokens (-1 = auto: math500→2000, others→4000)')
    parser.add_argument('--dataset', type=str, choices=['math500', 'aime24', 'aime25', 'aime24_aime25', 'mbpp', 'mbpp_train_test', 'gpqa_diamond'],
        default='aime24_aime25', help='Dataset to collect from')
    parser.add_argument('--output_file', type=str, default=None,
        help='Output file (default: collected_sae_latents_8b_{n_clusters}dim_{max_token}_{dataset}.pt)')
    ...
    DEFAULT_MAX_TOKENS = {
        ...
        "gpqa_diamond": 4000,
        "mbpp": 4000,
        "mbpp_train_test": 4000,
    }
    if args.max_token == -1:
        args.max_token = DEFAULT_MAX_TOKENS.get(args.dataset, 4000)
        print(f"ℹ️  max_token not specified, using default {args.max_token} for dataset '{args.dataset}'")

    print(f"Loading Model & SAE...")
    model, tokenizer = load_model(model_name=args.model, device="cuda", load_in_8bit=args.load_in_8bit)
    hf_model = model._model  # 直接访问底层 HuggingFace 模型，避免 nnsight trace 冲突
    ...
    sae, _ = load_sae("deepseek-r1-distill-llama-8b", args.sae_layer, args.n_clusters, require_activation_mean=False)
    ...
    if args.output_file is None:
        args.output_file = f"collected_sae_latents_8b_{args.n_clusters}dim_{args.max_token}_{args.dataset}.pt"
    ...
    hf_model.model.layers[args.sae_layer].register_forward_hook(collector)
    ...
    def get_item_fields(item, dataset_name):
        if dataset_name in ("mbpp", "mbpp_train_test"):
            ...
            ans = solution.replace("Answer:", "").strip().upper() if solution else ""
            if len(ans) > 1:
                ans = ans[0]
            return question, solution, ans, "mcqa", None
        else:
    ...
    for i in target_indices:
        ...
        with torch.no_grad():
            out = hf_model.generate(**inputs, max_new_tokens=args.max_token, do_sample=False)
